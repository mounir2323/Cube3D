...
void render_wall(t_mlx *mlx, int ray) // render the wall
{
 ...
 wall_h = (TILE_SIZE / mlx->ray->distance) * ((WIDTH / 2) / tan(mlx->ply->fov_rd / 2)); // get the wall height
...
void cast_rays(t_mlx *mlx) // cast the rays
{
 ...
 ray = 0;
 mlx->ray->ray_ngl = mlx->ply->init_direction - (mlx->ply->fov_rd / 2); // the start angle
 while (ray < WIDTH) // loop for the rays
 {
  ...
  ray++; // next ray
  mlx->ray->ray_ngl += (mlx->ply->fov_rd / WIDTH); // next angle
 }
}
